...
corpus = pd.read_json("./datasets/get-indexed-songs.json");
user_watch_history = pd.read_json("./training-get-history.json");
user_search_history = pd.read_json("./training-search.json")
...
features_embedding_layer = tf.keras.layers.Embedding(input_dim=NUM_CLASSES, output_dim=EMBEDDING_DIMS, mask_zero=True, trainable=True, name='searched_embeddings')
labels_embedding_layer = tf.keras.layers.Embedding(input_dim=NUM_CLASSES, output_dim=EMBEDDING_DIMS, mask_zero=True, trainable=True, name='watched_embeddings')
...
], user_watched_group['past_predicted'].values, callbacks=[tensorboard_callback], steps_per_epoch=1, epochs=100, verbose=1)
