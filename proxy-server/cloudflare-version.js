...
const RequestHandler = async (event) => {
    ...
    const fetch_url = unescape(unescape(origin.search.substr(1)));
    const orig = event.request.headers.get("Origin");
    console.log(origin, orig, fetch_url)

    if (preflight) {

    }
    if ((!isListed(fetch_url, blacklist)) && (isListed(origin, whitelist))) {

    }
}
addEventListener("fetch", async event => {
    event.respondWith(await RequestHandler(event));
});
