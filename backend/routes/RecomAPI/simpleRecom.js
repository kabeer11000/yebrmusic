...
router.post('/backend/discover/deprecated/knn', async (req, res) => {
    ...
    try {
        ...
        // await axios.get("http://localhost:9000/recom/generate-candidates");
        ...
        const recommendations = await axios.post('http://localhost:5000/feed/recommendations/deprecated/knn', {
...
router.get("/generate-candidates", async (req, res) => {
    const predictions = await axios.get("http://127.0.0.1:5000/next-song-prediction/get-all-predictions").then(a => a.data);
...
router.get("/playlist/:id", async (req, res) => {
    ...
    // }));
    // res.json(videos);
    res.json((await scraper.getPlayList(req.params.id)).items);
});
router.get("/concat/playlist/:ids", async (req, res) => {
    ...
    for (const id of ids) {
        ...
        const list = await scraper.getPlayList(id);
        response = [...response, list.items.map(song => ({}))]
    }
...
router.get('/tests/get-candidates/:user_id', async (req, res) => {
    const db = await MongoClient;
