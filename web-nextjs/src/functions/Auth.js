...
import {fetchWithTimeout} from "./Helper/Utils";
...
export const GetActiveAccounts = async () => {
    const accounts = await fetchWithTimeout(endPoints.Auth.GetActiveSessionAccounts /* "http://localhost:8084/service/user/sessions" */, {
        method: "post",
        credentials: "include",
        timeout: 45000
    }).then(a => a.status === 200 ? a.json() : []).catch(() => []);
