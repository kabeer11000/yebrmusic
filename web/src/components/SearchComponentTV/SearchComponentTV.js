...
const EmptyState = () => (
    <Grow in={true}>
        <div className={"errorPage text-center"}
            ...
            {/*<br/>*/}
            <Typography
                variant="button">{navigator.onLine ? "Results will appear as you type" : "Searching In Downloads"}</Typography>
        </div>
    </Grow>
);
const SearchComponentTV = () => {
    ...
    const Search = async (query, enter = false) => {
        setQuery(query);
        try {
            if (online) {
                setPredictions(await SuggestSearch(query));
                if (query.length >= 10 || enter) {
